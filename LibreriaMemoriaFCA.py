...
def save_result(results,name,path):
	d = datetime.now()
	timestamp = '{}{}{}-{}-{}-{}'.format(d.year, d.month, d.day, d.hour, d.minute, d.second)
	fname = path[path.rfind('/'):path.rfind('.')]
	fname=fname[1:]
	with open('results/{}-{}-{}.json'.format(name,fname, timestamp) , 'w') as fout:
	    json.dump(results, fout)

def algorithm_fca(dataset,algoritmo):
	from time import time
	start_time = time() 
	path = dataset
	...
	if algoritmo=="apriori":
		...
		save_result(results,"apriori",path)